...
## 📌 Executive Summary
**iTantra** is a serverless, zero-internet Peer-to-Peer (P2P) tactical walkie-talkie and transceiver application engineered for extreme environments, disaster zones, and tactical field operations. It enables real-time Speech-to-Text (STT), cross-lingual translation, and Text-to-Speech (TTS) playback across the **10 mandatory ISRO languages** over zero-infrastructure local Wi-Fi Hotspots and BLE Mesh networks.
...
| **Packet Protocol & CRC32** | ✅ **PRODUCTION GRADE** | **Mode 1** 32-bit (4-byte fixed intent) payloads + **Mode 2** dynamic dynamic payload framing with IEEE 802.3 CRC32 integrity checks. |
...
| **Telemetry & Benchmarking** | ✅ **PASSED** | Replaced static delays with high-resolution `Stopwatch()` timers measuring real execution metrics. |
...
* **Offline NLU & Translation**: Multi-lingual script detection engine, `OfflineIndicTranslator`, and English Pivot translation architecture.
...
5. **PTT Speech**: Hold the circular **PTT Button** on Device A and speak. Device B instantly receives, translates to its chosen target language (e.g. Odia, Hindi, Tamil), and speaks the translation aloud!
